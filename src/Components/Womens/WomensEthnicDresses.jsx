...
const WomensEthnicDresses = () => {
  ...
  const AddToCartItem = async (id) => {
    ...
    let data = await axios.get(
      `https://alok-verma-rct.onrender.com/WomensEthnicWear/${id}`
    );
  ...
  return (
    <Flex direction={["column", "row"]} justifyContent="space-between">
      <Card className={styles.prodsort} m="10px 0px" p="20px">
        ...
        <Center>
        <RadioGroup defaultValue="ethnic Dresses">
            <Stack direction='column' textAlign={"left"}>
                <Link to="/Womens/WomensDresses"><Radio value={"dresses"}>Dresses</Radio></Link>
                <Link to="/Womens/WomensFootwear"><Radio value={"footwear"}>Footwear</Radio></Link>
                <Radio value={"ethnic Dresses"}>Ethnic Dresses</Radio>
            </Stack>
        </RadioGroup>
        </Center>
        {/* <Menu p="30px">
