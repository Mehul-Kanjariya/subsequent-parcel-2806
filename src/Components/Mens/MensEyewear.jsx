...
const MensClothing = () => {
  ...
  const FetchData = async () => {
    ...
    try {
      let res = await axios.get(
        "https://alok-verma-rct.onrender.com/MensEyewear"
      );
      let data = res.data;
  ...
  const AddToCartItem = async (id) => {
    ...
    let data = await axios.get(
      `https://alok-verma-rct.onrender.com/MensEyewear/${id}`
    );
  ...
  return (
    <Flex direction={["column", "row"]} justifyContent="space-between">
      <Card className={styles.prodsort} m="10px 0px"  p="20px">
        ...
        <Center>
        <RadioGroup defaultValue="eyewear">
            <Stack direction='column' textAlign={"left"}>
                <Link to="/Mens/MensClothing"><Radio value={"clothing"}>Clothing</Radio></Link>
                <Link to="/Mens/MensFootwear"><Radio value={"footwear"}>Footwear</Radio></Link>
                <Radio value={"eyewear"}>Eyewear</Radio>
            </Stack>
        </RadioGroup>
        </Center>
        {/* <Menu p="30px">
      ...
      <SimpleGrid minChildWidth={250} m="20px" p="10px" w="80%">
        {loading ? (
          ...
          products?.map((item) => {
            return (
              <Link to={`/Mens/MensEyewear/${item.id}`}>
              <Flex
                p={5}
                w="fit-content"
                alignItems="center"
                ...
                >
                  ...
                  >
                    <Box
                      ...
                      onClick={() => {
                        navigate(`/Mens/MensEyewear/${item.id}`);
                      }}
                    >
                      {item.title}
                    </Box>
